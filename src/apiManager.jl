...
- `filepath::AbstractString` = Path where to save the obtained `.json`
  file from the API available in CDSA.

# Notes

The API requires the definition of `parentType` and `parentId`.
`parentId` specifies the identifier of the collection. The collection
of images associated with TCGA is identified by the code:
`5b9ef8e3e62914002e454c39`.  The use of `limit=0` sets the absence of
limits in the queried file, ensuring the complete download of the
...
Function to extract the values of data collections from the `.json`
file downloaded by the `download_collection_values` function.
...
Function to download metadata associated with the selected collection
at startup.

# Arguments

- `filepath::AbstractString` = Path to save the `.json` file
   associated with the collection. The file is indicated with the
   wording `collection_name.json`.
- `collection_name::AbstractString` = Name of the collection from
   which to download the slides.

# Notes

The API requires the definition of `parentType`, `parentId`, and
`name`.  The `name` attribute identifies the name of the collection
from which you want to retrieve data (e.g., chol, esca, etc.). The API
belongs to the category for managing the folders stored in the
...
Function to extract the `id` value from the metadata of the collection
selected at startup.

# Arguments

- `filepath::AbstractString` = Path where the `collection_name.json`
  file is stored.
...
function extract_project_id(filepath::AbstractString)
    ...
    ## Read the project info file and insert into a variable the data
    ## of the project ID.
...
Function to download metadata associated with the cases of the
selected collection at startup.

# Arguments

- `filepath::AbstractString` = Path where to save the `.json` file
  associated with the cases of the collection. The file is indicated
  with the term `collection_name.json`.
...
Function for downloading histological slides in `.zip` format
associated with the cases of the selected collection at startup.
