...
Function for storing in the `JHistint_DB` database the information
associated with each slide downloaded from the Cancer Digital Slide
Archive (CDSA).

# Arguments

- `col_name::AbstractString` = Collection name.
- `cas_name::AbstractString` = Case name, which corresponds to the
  `CASE-NAME` displayed by the package.
...
The `JHistint_DB` database is used for storing the information
associated with each slide downloaded from the CDSA.

The function takes a dictionary containing the information associated
with the slide and stores it in the database.
...
- `slide_info_participant_code TEXT` = Slide information - Participant Code,
  alphanumeric string.
- `slide_info_sample_type TEXT` = Slide information - Sample Type. The
  values associated with tumor samples are in the range 01-09. 10-19
  indicates the range for non-diseased normal samples. 20-29 indicates
  samples currently under control.
- `slide_info_vial TEXT` = Slide information - Vial. Related to the
  ordering of the sample in the sequence of samples. The values range
  from A-Z.
- `slide_info_portion TEXT` = Slide information - Portion. Related to
  the ordering of the analyzed portions associated with a sample. Takes
  values in the range 01-99.
- `slide_info_type TEXT` = Slide information - Image Type. The
  possible values are TS (Top Slide), BS (Bottom Slide), and MS (Middle Slide).
...
function insert_record_DB(col_name::AbstractString,
    ...
    SQLite.execute(db,
                   "CREATE TABLE IF NOT EXISTS Slide(collection_name TEXT,
                                                     case_name TEXT,
                                                     TCGA_caseID TEXT,
                                                     slide_ID TEXT,
...
Function for storing in the `JHistint_DB` database the information
associated with each slide downloaded from the Cancer Digital Slide
Archive (CDSA).  The function is different from the standard model. It
stores only informations of the downloaded Slides.
...
- `slide_path_folder_svs TEXT` = Path where the `.tif` file is stored.
- `slide_path_api TEXT` = Link to the API for downloading the slide.
- `slide_path_folder_seg TEXT` = Path where the segmented `.tif` file
  is stored.
- `slide_svs BLOB` = Histopathological slide (image).
- `slide_info_TSS TEXT` = Slide information - Tissue Source Site.
- `slide_info_participant_code TEXT` = Slide information - Participant
  Code, alphanumeric string.
- `slide_info_sample_type TEXT` = Slide information - Sample Type.
  The values associated with tumor samples are in the range
  01-09. 10-19 indicates the range for non-diseased normal
  samples. 20-29 indicates samples currently under control.
- `slide_info_vial TEXT` = Slide information - Vial. Related to the
  ordering of the sample in the sequence of samples. The values range from A-Z.
- `slide_info_portion TEXT` = Slide information - Portion. Related to
  the ordering of the analyzed portions associated with a sample. Takes
  values in the range 01-99.
- `slide_info_type TEXT` = Slide information - Image Type. The
  possible values are TS (Top Slide), BS (Bottom Slide), and MS (Middle
  Slide).  The alphanumeric value indicates the ordering of the slide.
...
- `collection_name::AbstractString`: Name of the slide collection to
  search for in the `JHistint_DB`.


# Return value

- `slide_list`: List of tuples, each of which contains the ID of the
  slide, the `.svs` file of the slide, and the path of the folder
  containing the `.svs` file.
"""
...
The function updates the `JHistint_DB` with the path of the segmented
image file, the path of the adjacency matrix file in text format, and
the matrix itself.


# Arguments

- `filepath_seg::AbstractString`: Path of the segmented image file to
  add to the DB.
- `filepath_matrix::AbstractString`: Path of the adjacency matrix file.
- `matrix::Matrix{Int64}`: Adjacency matrix.
- `slide_id::AbstractString`: ID of the slide to update with the
  segmented image information.
"""
function load_seg_slide(filepath_seg::AbstractString,
